...
class Simulator(object):
    def __init__(
        ...
        parameters=None,
        compile_args=None,
        compile_args_vhdl=None,
        compile_args_verilog=None,
        sim_args=None,
        extra_args=None,
        plus_args=None,
        force_compile=False,
        ...
        **kwargs
        ...
        if compile_args_verilog is None:
            compile_args_verilog = []

        if compile_args_vhdl is None:
            compile_args_vhdl = []
        ...
        self.compile_args_vhdl = compile_args_vhdl
        self.compile_args_verilog = compile_args_verilog
...
class Questa(Simulator):
    ...
    def get_parameter_commands(self, parameters):
        parameters_cmd = []
        for name, value in parameters.items():
            # parameters_cmd.append("-g" + name + "=" + str(value))  # Original
            parameters_cmd.append("-G" + name + "=" + str(value))  # Modified
    ...
    def build_command(self):
        ...
        if self.vhdl_sources:
            do_script = "vlib {RTL_LIBRARY}; vcom -mixedsvvh {FORCE} -work {RTL_LIBRARY} {EXTRA_ARGS} {VHDL_SOURCES}; quit".format(
                ...
                EXTRA_ARGS=" ".join(as_tcl_value(v) for v in self.compile_args).join(" ").join(as_tcl_value(v) for v in self.compile_args_vhdl),
        ...
        if self.verilog_sources:
            do_script = "vlib {RTL_LIBRARY}; vlog -mixedsvvh {FORCE} -work {RTL_LIBRARY} +define+COCOTB_SIM -sv {DEFINES} {INCDIR} {EXTRA_ARGS} {VERILOG_SOURCES}; quit".format(
                ...
                EXTRA_ARGS=" ".join(as_tcl_value(v) for v in self.compile_args).join(" ").join(as_tcl_value(v) for v in self.compile_args_verilog),
                FORCE= "" if self.force_compile else "-incr",
            )
            cmd.append(["vsim"] + ["-c"] + ["-do"] + [do_script])
